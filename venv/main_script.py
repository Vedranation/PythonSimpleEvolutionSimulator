...
World_type_list_x_y = [[None for _ in range(world_size)] for _ in range(world_size)] #stores types of Agents (plant, herbivore, carnivore...)
World_agent_list_x_y = [[None for _ in range(world_size)] for _ in range(world_size)] #stores specific agent instances
class Agent:
    ...
    def FindFreeSpot(self):
        ...
        while World_type_list_x_y[self.x][self.y] != None:
            self.x = random.randint(0, world_size-1)
            self.y = random.randint(0, world_size-1)
        World_type_list_x_y[self.x][self.y] = self.type
    ...
    def RemoveAgent(agent):
        ...
        """
        World_type_list_x_y[agent.x][agent.y] = None
        ...
        if self.perception >= 1: #lowest perception, only sees up down left right
            ...
            for direction in directions_x_y:
                if direction[0] >= world_size or direction[1] >= world_size or direction[0] < 0 or direction[1] < 0: #prevents checking beyond edge of world
                    continue
                print(f"{self.name} X: {self.x} Y: {self.y}  |  check X: {direction[0]} Y:{direction[1]}  |  found: {World_type_list_x_y[direction[0]][direction[1]]}") #console log

                #if we found food, eat it and go there:
                if World_type_list_x_y[direction[0]][direction[1]] == self.food:
                    ...
                    World_type_list_x_y[direction[0]][direction[1]] = self.type
                    World_type_list_x_y[self.x][self.y] = None
